...
struct gl3_backend::pimpl
{
    ...
    //****************************************************************************************
    void_t handle_render_state( natus::graphics::render_state_sets_cref_t new_states, bool_t const popped = false )
    {
        // depth test
        {
        ...
        // polygon mode
        {
            if( new_states.polygon_s.do_change )
            {
                if( new_states.polygon_s.ss.do_activate )
                {
                    ...
                    glCullFace( natus::graphics::gl3::convert( new_states.polygon_s.ss.cm ) ) ;
                    natus::ogl::error::check_and_log( natus_log_fn( "glCullFace" ) ) ;

                    glFrontFace( natus::graphics::gl3::convert(new_states.polygon_s.ss.ff ) ) ;
                    natus::ogl::error::check_and_log( natus_log_fn( "glFrontFace" ) ) ;

                    glPolygonMode( GL_FRONT_AND_BACK, natus::graphics::gl3::convert( new_states.polygon_s.ss.fm ) ) ;
    ...
    bool_t deactivate_framebuffer( void_t ) noexcept
    {
        ...
        {
            glBindFramebuffer( GL_FRAMEBUFFER, 0 ) ;
            if( !natus::ogl::error::check_and_log( natus_log_fn( "glGenFramebuffers" ) ) ) return false ;
        }
    ...
    //****************************************************************************************
    size_t construct_shader_config( size_t oid, natus::graphics::shader_object_ref_t obj ) noexcept
    {
    ...
    bool_t release_shader_data( size_t const oid ) noexcept
    {
        ...
        {
            natus::memory::global_t::dealloc_raw( shd.output_names ) ;
        }
        ...
        natus::graphics::image_object_ref_t config )
    {
        ...
        {
            if( natus::core::is_not( _images[ i ].tex_id != GLuint( -1 ) ) )
            {
                break ;
            }
        }
        ...
        {
            _images[ i ].name = name ;
            _images[ i ].type = natus::graphics::gl3::convert( config.get_type() ) ;
        }

        {
            for( size_t j=0; j<(size_t)natus::graphics::texture_wrap_mode::size; ++j )
            {
                _images[ i ].wrap_types[ j ] = natus::graphics::gl3::convert(
                    config.get_wrap( ( natus::graphics::texture_wrap_mode )j ) );
            }

            for( size_t j = 0; j < ( size_t ) natus::graphics::texture_filter_mode::size; ++j )
            {
                _images[ i ].filter_types[ j ] = natus::graphics::gl3::convert(
    ...
    //****************************************************************************************
    bool_t update( size_t const id, natus::graphics::render_object_ref_t rc ) noexcept
    {
    ...
    {
        oid = determine_oid( obj.name(), _geometries ) ;
        ...
        if( is_config || ( sib == 0 || config.sib < sib ) )
        {
            ...
            else if( target == GL_TEXTURE_2D_ARRAY )
            {
                glTexImage3D( target, level, internal_format, width, height, depth,
                    border, format, type, data ) ;
                natus::ogl::error::check_and_log( natus_log_fn( "glTexImage3D" ) ) ;
            }
        }
        else
        {
    ...
    //****************************************************************************************
    size_t construct_tf_data( size_t oid, natus::graphics::streamout_object_ref_t obj ) noexcept
    {
        oid = determine_oid( obj.name(), _feedbacks ) ;
        ...
        for( size_t i=0; i<req_buffers; ++i )
        {
            ...
            GLuint id = GLuint( -1 ) ;
            glGenBuffers( 2, buffer.ids ) ;
            error = natus::ogl::error::check_and_log( 
                natus_log_fn( "[construct_tf_data] : glGenBuffers" ) ) ;

            data.buffers[i].sib[0] = 0 ;
            data.buffers[i].sib[1] = 0 ;
        }

        for( size_t i=req_buffers; i<this_t::tf_data::max_buffers; ++i )
        {
            if( data.buffers[i].ids[0] == GLuint(-1) ) break ;
            glDeleteBuffers( 2, data.buffers[i].ids ) ;
            natus::ogl::error::check_and_log( natus_log_fn( "glDeleteBuffers" ) ) ;
    ...
    bool_t release_tf_data( size_t const oid ) noexcept
    {
        ...
        {
            if( d.buffers[i].ids[0] == GLuint( -1 ) ) break ;

            glDeleteBuffers( 2, d.buffers[i].ids ) ;
            natus::ogl::error::check_and_log( natus_log_fn( "glDeleteBuffers" ) ) ;
    ...
    //****************************************************************************************
    bool_t update( size_t oid, natus::graphics::array_object_ref_t obj, bool_t const is_config = false ) 
    {
        auto & data = _arrays[ oid ] ;
    ...
    bool_t update( size_t oid, natus::graphics::streamout_object_ref_t obj, bool_t const is_config = false ) 
    {
        ...
        while( data.buffers[i].ids[0] != GLuint(-1) )
        {
            for( size_t j=0; j<2; ++j )
            {
                ...
                // allocate data
                GLuint const sib = GLuint( obj.buffer( i ).get_layout_sib() * obj.size() ) ;
        ...
        GLsizei const num_elements = GLsizei(-1) )
    {
        ...
        if( feed_from_tf )
        {
            if( config.tf_id != size_t( -1 ) )
            {
                this_t::tf_data & tfd = _feedbacks[ config.tf_id ] ;
                glBindBuffer( GL_ARRAY_BUFFER, tfd.buffers[0].ids[ tfd.read_index() ] ) ;
                if( natus::ogl::error::check_and_log( natus_log_fn("glBindBuffer(GL_ARRAY_BUFFER)") ) ) 
                    return false ;
            }
            else
            {
                natus::log::global_t::warning( "want to feed from streamout buffer but no such buffer bound. "
                    "Keeping geometry bound.") ;
        ...
        // render section
        {
            ...
            if( feed_from_tf && config.tf_id != size_t( -1 ) )
            {
                GLuint num_prims = 0 ;
                {
                    auto & tfd = _feedbacks[ config.tf_id ] ;
...
natus::graphics::result gl3_backend::configure( natus::graphics::streamout_object_res_t obj ) noexcept 
{
    ...
    {
        id->set_oid( this_t::get_bid(), _pimpl->construct_tf_data( 
            id->get_oid( this_t::get_bid() ), *obj ) ) ;
    }
