    ...
    if( ImGui::BeginListBox( label ) )
    {
        for( size_t i=0; i<items.size(); ++i )
        {
...

            {
                ...
                else if( natus::tool::imgui_custom::Selectable( m, &selected, &hovered, ImGuiSelectableFlags_AllowDoubleClick, item_dims ) )
                {
                    if( ImGui::IsMouseDoubleClicked( ImGuiMouseButton_Left ) )
                    {
                        double_clicked = i ;
            ...
            if( double_clicked != i )
            {
                ...
                ImGui::SameLine() ;
                natus::ntd::string_t const name = natus::ntd::string_t( label ) + "##input_int" + std::to_string(i) ;
...
bool natus::tool::imgui_custom::Selectable(const char* label, bool* p_selected, bool_t * p_hovered, ImGuiSelectableFlags flags , const ImVec2& size_arg )
{
