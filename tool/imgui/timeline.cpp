...
bool_t timeline::begin( natus::ntd::string_cref_t label, natus::tool::imgui_view_t ) noexcept 
{
    ...
    float_t const mouse_x = ImGui::GetMousePos().x - ImGui::GetWindowPos().x + scroll_x ;
    ...
    // draw time line
    {
        ...
        ImVec2 const start = ImGui::GetCursorScreenPos() + ImVec2( scroll_x - offset_x, 10.0f ) ;
        ...
        // draw small lines
        {
            ...
            for( size_t i=0; i<num_steps; ++i )
            {
                ...
                ImVec2 const p1 = start + ImVec2( small * float_t(i), height * 0.25f ) ;
        ...
        // draw big lines
        {
            ...
            for( size_t i=0; i<num_steps; ++i )
            {
                ...
                ImVec2 const p1 = start + ImVec2( big * float_t(i), height * 0.5f ) ;
    ...
    // testing line moving with mouse
    {
        ImGui::SetCursorScreenPos( ImGui::GetCursorScreenPos() + ImVec2( mouse_x, 0.0f ) ) ;

        {
            ImVec2 pos = ImGui::GetCursorScreenPos() ;
            draw_list->AddLine( pos + ImVec2(0.0f,0.0f), pos + ImVec2( 0.0f,10.0f ), IM_COL32(255, 255, 0, 255), 1.0f ) ;
        }
    }
