...
bool_t custom_imgui_widgets::direction( char const * label, natus::math::vec2f_ref_t dir ) noexcept
{
    bool_t ret = false ;
    ...
    if( is_active )
    {
        natus::math::vec2f_t const c( center.x, center.y ) ;
        natus::math::vec2f_t const m( io.MousePos.x, io.MousePos.y ) ;

        dir = (m - c).normalize() * natus::math::vec2f_t( 1.0f, -1.0f ) ;
    ...
    // do the drawing
    {
        ...
        // the line
        {
            auto const dirn = dir.normalized() * natus::math::vec2f_t( 1.0f, -1.0f ) ;

            auto const inner = natus::math::vec2f_t( center.x, center.y ) + dirn * (radius_inner + 0 ) ;
            auto const outer = natus::math::vec2f_t( center.x, center.y ) + dirn * (radius_outer - 2 ) ;
