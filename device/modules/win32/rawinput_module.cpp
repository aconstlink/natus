...
//***
void_t rawinput_module::search( natus::device::imodule::search_funk_t funk )
{
    funk( _three_device ) ;
    funk( _ascii_device ) ;
}
    ...
    if( raw->header.dwType == RIM_TYPEMOUSE )
    {
        ...
        if( raw->data.mouse.usFlags == MOUSE_MOVE_RELATIVE )
        {
            bs = ( last_pt.x == 0 && last_pt.y == 0 ) ?
                natus::device::components::button_state::pressed :
                natus::device::components::button_state::none ;
        }
