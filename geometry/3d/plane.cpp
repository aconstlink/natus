...
//*************************************************************************************
natus::geometry::result plane::make( polygon_mesh_ptr_t mesh_ptr, input_params_cref_t ip ) 
{
    if( natus::core::is_nullptr(mesh_ptr) )
    ...
    natus::geometry::polygon_mesh_ref_t m = *mesh_ptr ;
...
void_t plane::construct_vertices( data_ref_t d, input_params_cref_t ip ) 
{
    ...
    // vertices
    {
        ...
        for(size_t iy = 0; iy < per_dim_verts; ++iy)
        {
            ...
            for(size_t ix = 0; ix < per_dim_verts; ++ix)
            {
                ...
                natus::math::vec3f_t v{ x*ip.scale.x(), y*ip.scale.y(), 0.0f } ;                
                natus::math::vec2f_t t{ tx, ty } ;
    ...
    // normals
    {
        ...
        natus::math::vec3f_t n{ 0.0f, 0.0f, -1.0f } ;
