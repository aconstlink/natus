...
{
    size_t const num_components = natus::geometry::vector_component_format_to_number( position_format ) ;
    ...
    if( normals.size() > 0 )
    {
        size_t const num_normal_components = natus::geometry::vector_component_format_to_number( normal_format ) ;
        if(num_components == 0)
            return false ;
    ...
    if( texcoords.size() > 0 )
    {
        size_t texcoord_components = natus::geometry::texcoords_component_format_to_number( texcoord_format ) ;
...
{
    size_t const num_components = natus::geometry::vector_component_format_to_number( normal_format ) ;
    if( num_components == 0 )
        return 0 ;
...
{
    size_t const num_components = natus::geometry::texcoords_component_format_to_number( texcoord_format) ;
    if( num_components == 0 )
        return 0 ;
...
//*******************************************************************
natus::math::vec3f_t flat_tri_mesh::get_vertex_position_3d( size_t const i ) const noexcept 
{
    size_t const index = i * 3 ;

    if( index+2 >= positions.size() ) return natus::math::vec3f_t( 0.0f ) ;
    return natus::math::vec3f_t( positions[ index + 0 ], positions[ index + 1 ], positions[ index + 2 ] ) ;
}

//*******************************************************************
natus::math::vec3f_t flat_tri_mesh::get_vertex_normal_3d( size_t const i ) const noexcept 
{
    size_t const index = i * 3 ;
    
    if( i+2 >= normals.size() ) return natus::math::vec3f_t( 0.0f ) ;
    return natus::math::vec3f_t( normals[ index + 0 ], normals[ index + 1 ], normals[ index + 2 ] ) ;
}

//*******************************************************************
natus::math::vec2f_t flat_tri_mesh::get_vertex_texcoord( size_t const l, size_t const i ) const noexcept 
{
    size_t const index = i * 2 ;

    if( l >= texcoords.size() || index+2 >= texcoords[ l ].size() ) return natus::math::vec2f_t() ;
    return natus::math::vec2f_t( texcoords[ l ][ index + 0 ], texcoords[ l ][ index + 1 ] ) ;
}
