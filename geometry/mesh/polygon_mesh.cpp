...
//*******************************************************************
natus::geometry::result polygon_mesh::flatten( tri_mesh_ref_t mesh_out ) const
{
    tri_mesh the_mesh ;
    ...
    return natus::geometry::ok ;
}

//*******************************************************************
natus::geometry::result polygon_mesh::flatten( flat_tri_mesh_ref_t mesh_out ) const
{
    tri_mesh_t mesh_a ;
...
polygon_mesh::this_t polygon_mesh::repeat( size_t const times ) const 
{
    ...
    // repeat polygons
    {
        natus::geometry::uints_t old_polys = std::move( m.polygons ) ;
        natus::geometry::uints_t new_polys( times * old_polys.size() ) ;
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.indices ) ;
        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.normals_indices[k] ) ;
        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.texcoords_indices[k] ) ;
        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
...
polygon_mesh::this_t polygon_mesh::repeat_full( size_t const times ) const 
{
    ...
    if( natus::log::global::error(m.position_format == natus::geometry::vector_component_format::invalid, 
    ...
    if( m.normals_indices.size() > 0 && 
        natus::log::global::error(m.normal_format == natus::geometry::vector_component_format::invalid, 
    ...
    if( m.texcoords_indices.size() > 0 &&
        natus::log::global::error( m.texcoord_format == natus::geometry::texcoord_component_format::invalid, 
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.indices ) ;
        natus::geometry::floats_t old_pos = std::move( m.positions ) ;

        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
        natus::geometry::floats_t new_pos( times * old_pos.size() ) ;

        size_t const div = natus::geometry::vector_component_format_to_number(m.position_format) ;
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.normals_indices[k] ) ;
        natus::geometry::floats_t old_normals = std::move( m.normals[k] ) ;

        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
        natus::geometry::floats_t new_normals( times * old_normals.size() ) ;

        size_t const div = natus::geometry::vector_component_format_to_number(m.normal_format) ;
    ...
    {
        natus::geometry::uints_t old_indices = std::move( m.texcoords_indices[k] ) ;
        natus::geometry::floats_t old_texcoords = std::move( m.texcoords[k] ) ;

        natus::geometry::uints_t new_indices( times * old_indices.size() ) ;
        natus::geometry::floats_t new_texcoords( times * old_texcoords.size() ) ;

        size_t const div = natus::geometry::texcoords_component_format_to_number(m.texcoord_format) ;
