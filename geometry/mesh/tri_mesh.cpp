...
{
    natus_this_typedefs( per_vertex_index_vector ) ;
...
typedef std::vector< per_vertex_index_vector > per_vertex_index_vectors_t ;
typedef std::vector< per_vertex_index_vectors_t > more_per_vertex_index_vectors_t ;
...
//*******************************************************************
natus::geometry::result tri_mesh::flatten( flat_tri_mesh_ref_t mesh_out ) const 
{    
    // do some checks
    {}

    size_t const num_position_components = 
        natus::geometry::vector_component_format_to_number( natus::geometry::vector_component_format::xyz ) ;
    ...
    //
    {
        std::vector< float_t > new_positions( num_new_vertices * 3 ) ;
        std::vector< float_t > new_normals( num_new_normals * 3 ) ;
        std::vector< float_t > new_uvs0( num_new_tex0 * 2 ) ;
        std::vector< float_t > new_uvs1( num_new_tex1 * 2 ) ;
        std::vector< float_t > new_uvs2( num_new_tex2 * 2 ) ;

        mesh_out.position_format = natus::geometry::vector_component_format::xyz ;
        mesh_out.normal_format = natus::geometry::vector_component_format::xyz ;
        mesh_out.texcoord_format = natus::geometry::texcoord_component_format::uv ;

        size_t const uvb = texcoord_format == natus::geometry::texcoord_component_format::uvw ? 3 : 2 ;
