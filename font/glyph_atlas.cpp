...
glyph_atlas::~glyph_atlas( void_t )
{
    for( auto * iptr : _atlas )
        natus::memory::global_t::dealloc( iptr ) ;
}
...
{
    _atlas.push_back( natus::memory::global_t::alloc( std::move(img), "[glyph_atlas::add_image]" ) ) ;
    return true ;
}

//*******************************************************************
void_t glyph_atlas::map_face_id( natus::ntd::string_cref_t name, size_t const id ) noexcept 
{
...
//*******************************************************************
bool_t glyph_atlas::face_id_for_name( natus::ntd::string_cref_t name, size_t & id ) const noexcept 
{
    auto iter = std::find_if( _face_name_to_id.begin(), _face_name_to_id.end(), [&]( name_id_t const & nid )
    {
        return nid.first == name ;
