...
using namespace natus::font::stb ;
    ...
    size_t const max_img_width, size_t const max_img_height ) 
{
    ...
    natus_typedef( font_data ) ;
    natus_typedefs( natus::ntd::vector<font_data_t>, font_datas ) ;
...

    {
        ...
        {
            stbtt_fontinfo font ;
            uchar_ptr_t raw_data = natus::memory::global_t::alloc_raw<uchar_t>( fs.sib() ) ;
            memcpy( raw_data, fs.data(), fs.sib() ) ;

            int const stb_res = stbtt_InitFont( &font, raw_data,
                stbtt_GetFontOffsetForIndex( raw_data, 0 ) ) ;

            if( natus::log::global_t::error( stb_res == 0, natus_log_fn( "stbtt_InitFont" ) ) )
                continue ;
    ...
    natus::ntd::vector< internal_glyph_info > internal_glyh_infos ;
    ...
    for( auto const & bb : internal_glyh_infos )
    {
        ...
        
        {
            ...
            gi.code_point = bb.cp ;
            gi.image_id = channel ;
            gi.start = natus::math::vec2f_t( float_t(start_x) / float_t(iw), float_t(start_y) / float_t(ih) ) ;
            gi.dims = natus::math::vec2f_t( float_t(w) / float_t(iw), float_t(h) / float_t(ih) ) ;
            
            {
                int adv, bearing ;
                stbtt_GetCodepointHMetrics( &font, bb.cp, &adv, &bearing ) ;
                
                gi.adv = natus::math::vec2f_t((float_t(adv-bearing) / float_t(iw))*s, gi.dims.y()) ;
            }
    ...
    if( natus::core::is_not_nullptr( img.get_plane() ) )
    {
        ga.add_image( std::move( img ) ) ;
    }
    ...
    for( auto & fd : fonts )
    {
        natus::memory::global_t::dealloc_raw( fd.raw_data ) ;
    }
