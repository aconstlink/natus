...
namespace natus
{
    namespace math
    {
        namespace dsp
        {
            ...
            class fft
            {
                natus_this_typedefs( fft<T> ) ;

            public:

                natus_typedefs( std::complex<T>, complex ) ;
                natus_typedefs( std::valarray<float_t>, samples ) ;
                natus_typedefs( std::valarray<complex_t>, complexes ) ;

                typedef std::pair< T, size_t > sorted_item_t ;
                natus_typedefs( std::vector< sorted_item_t >, sorted ) ;
            ...
            public: // static 
                ...
                static void_t compute( complexes_inout_t x )
                {
                    ...
                    {
                        complex_t t = std::polar( T( 1.0 ), T( -2.0 ) * natus::math::constants<T>::pi() *
                            T( k ) / T( N ) ) * odd[ k ];
                        x[ k ] = even[ k ] + t;
                        x[ k + N / 2 ] = even[ k ] - t;
                    }
