...
{ \
    ...
    \
    natus::log::global_t::error( fn == NULL, "[CHECK_AND_LOAD] : Failed to load: "  name  ) ; \
}
...
{ \
    natus::ogl::fn = (decltype(natus::ogl::fn))(this_t::load_gl_function( name ) ) ; \
    natus::log::global_t::error( natus::ogl::fn == NULL, "[JUST_LOAD] : Failed to load: "  name  ) ; \
}
...
natus::ogl::result natus::ogl::gl::init( void_t ) 
{
    ...
    {
        GLint numext = 0 ;
        natus::ogl::glGetIntegerv( GL_NUM_EXTENSIONS, &numext  ) ;

        for( GLint i=0; i<numext; ++i )
        {
            GLubyte const * name = natus::ogl::glGetStringi( GL_EXTENSIONS, i  ) ;
